...
#> 
   ...
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "ms-identity-nodejs-webapi-azurefunctions" `
   ...
   Write-Host "Creating the AAD application (ms-identity-nodejs-webapi-azurefunctions)"
   # create the application 
   $serviceAadApplication = New-AzureADApplication -DisplayName "ms-identity-nodejs-webapi-azurefunctions" `
    ...
    if ($scopes.Count -ge 0) 
    {
        ...
        else 
        {
            ...
            $scope = CreateScope -value "access_as_user"  `
                -userConsentDisplayName "Access ms-identity-nodejs-webapi-azurefunctions"  `
                -userConsentDescription "Allow the application to access ms-identity-nodejs-webapi-azurefunctions on your behalf."  `
                -adminConsentDisplayName "Access ms-identity-nodejs-webapi-azurefunctions"  `
   ...
   Write-Host "Done creating the service application (ms-identity-nodejs-webapi-azurefunctions)"
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>ms-identity-nodejs-webapi-azurefunctions</a></td></tr>" -Path createdApps.html
   ...
   Write-Host "  - Navigate to the Manifest page, find the 'accessTokenAcceptedVersion' section and set it to '2'" -ForegroundColor Red 
